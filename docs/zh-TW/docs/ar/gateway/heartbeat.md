...
افتراضيًا، تُخفى إقرارات `HEARTBEAT_OK` بينما يُسلَّم محتوى التنبيه.
يمكنك ضبط ذلك لكل قناة أو لكل حساب:
...
أبقِ `HEARTBEAT.md` صغيرًا وفكّر في `model` أو `target: "none"` الأرخص
