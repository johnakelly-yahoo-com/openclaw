...
read_when:
  ...
  - "تريد مسار فرز أولي قبل التعمق في كتيبات التشغيل التفصيلية"
...
نفّذ هذا التسلسل حرفيًا وبالترتيب:
...
<AccordionGroup>
  ...
  <Accordion title="Gateway لا يبدأ أو أن الخدمة مُثبّتة لكنها لا تعمل">
    ```bash
    openclaw status
    openclaw gateway status
    openclaw logs --follow
  ...
  <Accordion title="لم يتم تشغيل Cron أو النبض (heartbeat) أو لم يتم التسليم">
    ```bash
    openclaw status
    openclaw gateway status
  ...
  <Accordion title="العُقدة مقترنة لكن الأداة تفشل في تنفيذ الكاميرا/اللوحة/الشاشة">
    ```bash
    openclaw status
    openclaw gateway status
