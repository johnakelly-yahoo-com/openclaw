...
عيّن رمز البوت عبر متغير البيئة (موصى به على الخوادم):
...
- ترث المواضيع (threads) إعدادات القناة الأم (قائمة السماح، `requireMention`، المهارات، المطالبات، إلخ) ما لم تُضِف معرّف قناة الموضوع صراحةً.
...
- **يتصل البوت لكنه لا يرد أبدًا في قناة خادم**:
  ...
  - تتطلب تهيئتك الإشارات ولم تُشر إليه، أو
...
| emojiUploads     | مفعّل     | رفع الإيموجي                                       |
...
| roles            | معطّل     | إضافة/إزالة الأدوار                                |
| moderation       | معطّل     | إسكات/طرد/حظر                                      |
| presence         | معطّل     | حالة/نشاط البوت (setPresence)   |
